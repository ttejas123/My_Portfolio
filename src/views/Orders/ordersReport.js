...
const columns = [
  ...
  },
  {
    ...
    minWidth: '180px',
            cell: row => {
              const data = row.response === true ? (<div>
                     --
                  </div>) : (<Badge color='danger' className='badge-glow'>
                    {row.requestFor}
                  </Badge>)
              return (
  ...
  const handleFilterByDropDown = (value) => {
    ...
    setSearchValue(search)
      if (search.length) {
          updatedData = salesOrderData.filter(item => {
           const orderId = item.orderId.toString()
            const startsWith =
              orderId.toLowerCase().startsWith(value.toLowerCase()) 
    ...
    <Fragment>
      <Card className='mx-1'>
        ...
        <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
          <CardTitle tag='h4'>Orders Report List</CardTitle>
