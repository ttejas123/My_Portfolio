...
import { AlignJustify, Rss, Info, Image, User, MapPin, Book, AtSign, Compass, Bell, StopCircle, Edit2, Edit3, FileText, Home, Award, Layers, Upload, Plus, Target, Percent, ArrowDownCircle, Box, ShoppingCart, Package, DollarSign, Truck, UserPlus, Star, Eye, Tool, Map } from 'react-feather'
...
const Bid = [
  {
    title: (<><Bell   size={17} style={{marginRight:"1px"}} /><span style={{fontSize: '14px'}}> Notifications </span></>),
    content: (
                    <ListGroup flush className='m-0 p-0'>
                      ...
                      <ListGroupItem className='m-0 p-0 cursor-pointer' style={activeTab === '2' ? {borderLeft: '1px solid red', paddingLeft: '6px'} : {paddingLeft: '6px'}} onClick={() => toggle1('2')}>
                        <Nav className='mb-0 ml-md-0 cursor-pointer'>
                          <NavItem style={{paddingLeft: '5px'}}>
                            <NavLink className='font-weight-bold'>
                              <span className='d-none d-md-block'>List Individuals</span>
                              <Home className='d-block d-md-none' size={14} />
                            </NavLink>
                          </NavItem>
                        </Nav>
  ...
  return (
    <Fragment className="addPadding m-0">
      <div style={{marginTop:"-1rem"}} className="ml-lg-4 mr-lg-5 pl-lg-3 pr-lg-3" >
        <BreadCrumbs  breadCrumbTitle='Notification' breadCrumbParent='Notification' breadCrumbChild='Dashboard' />
      </div>
      <div  >
